...
2. **Create the `reservations` table** using the ready-to-run SQL found in [`migrations/001_create_reservations_table.sql`](migrations/001_create_reservations_table.sql). The script defines typed columns (`date`/`time`) and an index on `(date, time)` for faster lookup:
